...
7. ⏳ **Chat type Claude.ai/Gemini** — décidé 2026-07-15/16. UX cible : bulles, texte qui s'écrit progressivement, sessions rechargeables, questions de suivi contextuelles. **Ordre décidé : 7a streaming → 7b persistance → 7c mémoire** (le barème d'abord, chaque étage testable). Le pipeline RAG 6a/6b/6c NE CHANGE PAS, on l'enveloppe.
   ...
   **7a — ✅ Streaming (valide le module LLM interface + effet Claude.ai) — FAIT et testé bout-en-bout (2026-07-16).** 287 tokens streamés un par un, sources émises avant le texte, `content-type: text/event-stream`. Bonus : la question MBPP vs SWE-bench qui échouait avant est désormais parfaitement répondue (chunking LangChain + re-ranking 6c ont réglé le fond).
   ...
   **7b — ✅ Persistance des conversations (sessions rechargeables) — FAIT et testé (2026-07-16).** Modèles Conversation+Message, migration `6fcd5d2f890f` (mergée main), `ConversationRepository`/`Service`/router CRUD (`/conversations`), fix autogenerate FK via `include_object` dans `env.py`. Branchement query : `/query/stream` accepte `conversation_id?`, crée la conversation si absent (titre auto), sauve le message user avant de streamer, accumule les tokens, sauve le message assistant + sources à la fin, émet le `conversation_id` en 1er event SSE. Testé : création + continuation d'une conversation, 4 messages chronologiques en base.
   ...
   **7c — ✅ Mémoire conversationnelle (questions de suivi) — FAIT et testé (2026-07-16).** Résumé glissant (`_build_history` : résume les vieux messages au-delà de 6, garde les récents bruts, recalculé à la volée), query rewriting (`_rewrite_question` : réécrit la question de suivi en question autonome via résumé + messages récents, AVANT le retrieval). `query_stream(data, history)` : rewrite → retrieve sur la question réécrite → génération avec historique injecté. **Test probant** : « What is MBPP? » puis « And how does it differ from the other one? » (« it »=MBPP, « the other one »=SWE-bench, jamais nommés) → le système résout les références et produit la comparaison MBPP vs SWE-bench. Le RAG conversationnel est complet.
